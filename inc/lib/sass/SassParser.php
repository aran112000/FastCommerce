...
class SassParser {
	...
	public function __construct($options = array()) {
		if (!is_array($options)) {
			if (isset($options['debug']) && $options['debug']) {
				throw new SassException('Options must be an array', $this);
			}
	...
	public function __get($name) {
		...
		if ($this->debug) {
			throw new SassException('No getter function for ' . $name, $this);
		}
	}
	 ...
	 * @param      $source
	 * @param bool $isFile
	 * @internal param string $name of source file or Sass source
	 * @return string CSS
	 */
	 ...
	 * The type of SassNode depends on the content of the SassToken.
	 *
	 * @param $node
	 * @throws SassException
	 ...
	 * @param $parent
	 * @throws SassException
	 * @internal param \token $SassToken to parse
	 * @internal param \parent $SassNode node
